...
def list_services_by_seller(seller_id: int, db: Session = Depends(get_db), skip: int = 0, limit: int = 20):
    service_layer = ServiceService(db)
    return service_layer.list_service_by_seller_id(seller_id, skip=skip, limit=limit)

@router.post("", response_model=ServiceResponse, status_code=status.HTTP_201_CREATED)
def create_service(data: ServiceCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    service_layer = ServiceService(db)
    return service_layer.create_service(seller_id=current_user.id, data=data)

@router.patch("/{service_id}", response_model=ServiceResponse)
def update_service(service_id: int, data: ServiceUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    service_layer = ServiceService(db)

    try:
        return service_layer.update_service(service_id=service_id, user_id=current_user.id, data=data)
    except LookupError as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error))
